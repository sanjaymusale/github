...
class LoadMoreGist extends React.Component {
  ...
  render() {
    const {
      results, data, isLoaded, inititalPage, lastPage, pagination, requestLoading, currentCard,
    } = this.state;
      ...
      <div className="load_all">
        <div className="pagination">
          {results.map((item, index) => {
            ...
            return (
              <div key={index} className="public_gist">
        ...
        <div className="pagination-action">
          ...
          {pagination.slice(inititalPage, lastPage + 1).map((item, index) => {
            ...
            if (currentPage === item) return <button key={index} className="active" id={item}>{item}</button>;
